...
This file provides guidance to AI coding agents (Claude Code, Codex, Cursor, and similar tools) working with code in this repository. It's the tool-neutral twin of `CLAUDE.md` — the two are kept in sync; if you update one, update the other.
...
`docs/` holds design history and planning docs, not living technical reference — check there for the *why* behind a decision (`DESIGN_VISION.md`, `MASTER_PLAN.md`, `ADAPTIVE_TUTOR_FINAL_PLAN.md`, `A1_CURRICULUM_REWORK_PLAN.md`), test philosophy (`TEST_STRATEGY.md`), or a release checklist (`RELEASE_CHECKLIST.md`) — but treat this file (and `CLAUDE.md`) and the code itself as the source of truth for current behavior.
